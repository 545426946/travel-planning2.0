# 微信登录失败排查指南

## 🚨 常见登录失败原因及解决方案

### 1. Edge Function 未部署 ❌

**问题表现：**
- 登录时提示 "服务器繁忙" 或 "网络连接失败"
- 控制台显示 Edge Function 请求失败

**解决方案：**
```bash
# 1. 登录 Supabase CLI
supabase login

# 2. 链接到你的项目
supabase link --project-ref hmnjuntvubqvbpeyqoxw

# 3. 部署 Edge Functions
supabase functions deploy wechat-login

# 4. 检查部署状态
supabase functions list
```

### 2. 微信小程序配置缺失 ❌

**问题表现：**
- AppID 为空或默认值
- 微信服务器返回 "无效的 AppID"

**解决方案：**

#### 步骤 1：获取微信小程序 AppID
1. 登录 [微信公众平台](https://mp.weixin.qq.com)
2. 进入小程序管理后台
3. 开发 → 开发管理 → 开发设置 → 开发者ID（AppID）

#### 步骤 2：配置 Edge Function 环境变量
```bash
# 设置微信小程序配置
supabase secrets set WECHAT_APP_ID="你的小程序AppID"
supabase secrets set WECHAT_APP_SECRET="你的小程序AppSecret"

# 验证环境变量
supabase secrets list
```

#### 步骤 3：更新项目配置
在微信开发者工具中：
1. `project.config.json` 确认 `appid` 正确
2. `app.json` 确认权限配置正确

### 3. 网络请求被阻止 ❌

**问题表现：**
- 控制台显示 "request:fail url not in domain list"
- HTTPS 证书错误

**解决方案：**

#### 微信小程序域名白名单配置
在微信公众平台添加以下域名：
- request合法域名：`https://hmnjuntvubqvbpeyqoxw.supabase.co`
- request合法域名：`https://api.weixin.qq.com`

#### 本地开发配置
在微信开发者工具：
- 详情 → 本地设置 → 不校验合法域名、web-view（业务域名）、TLS版本以及HTTPS证书

### 4. 数据库连接问题 ❌

**问题表现：**
- 数据库查询失败
- 用户信息无法保存

**解决方案：**
```sql
-- 检查表是否存在
SELECT table_name, table_schema 
FROM information_schema.tables 
WHERE table_schema IN ('public', 'auth');

-- 检查 RLS 策略
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual 
FROM pg_policies 
WHERE tablename IN ('users', 'user_sessions');
```

### 5. 代码逻辑错误 ❌

**常见错误及修复：**

#### 错误 1：模块导入失败
```javascript
// ❌ 错误
const { wechatLogin } = require('../../utils/wechat-login').wechatLogin

// ✅ 正确
const { wechatLogin } = require('../../utils/wechat-login')
```

#### 错误 2：API 调用错误
```javascript
// ❌ 错误 - 硬编码配置
const supabaseUrl = 'https://hmnjuntvubqvbpeyqoxw.supabase.co'

// ✅ 正确 - 从环境变量读取
const { supabase } = require('./supabase')
```

## 🔍 调试步骤

### 1. 检查 Edge Function 状态
```bash
supabase functions logs wechat-login
```

### 2. 检查环境变量
```bash
supabase secrets list
```

### 3. 测试数据库连接
```javascript
// 在控制台运行
const { supabase } = require('../../utils/supabase')
supabase.from('users').select('*').limit(1).then(console.log)
```

### 4. 测试微信登录流程
```javascript
// 在控制台运行
const { wechatLogin } = require('../../utils/wechat-login')
wechatLogin.login().then(console.log)
```

## 🛠️ 临时解决方案

如果 Edge Function 暂时无法使用，可以启用本地模式：

```javascript
// utils/wechat-login.js
async sendCodeToServer(code) {
  try {
    // 强制使用本地 fallback
    console.log('🔄 强制使用本地处理模式')
    return await this.localWechatLoginFallback(code)
  } catch (error) {
    console.error('❌ 本地登录也失败:', error)
    throw error
  }
}
```

## 📋 检查清单

- [ ] 微信小程序 AppID 已正确配置
- [ ] Edge Function 已部署
- [ ] 环境变量已设置
- [ ] 域名白名单已配置
- [ ] 数据库表已创建
- [ ] RLS 策略已配置
- [ ] 网络请求权限已开启

## 🚀 快速修复命令

```bash
# 1. 完整重新部署
supabase functions deploy wechat-login --no-verify-jwt

# 2. 设置所有必需的环境变量
supabase secrets set WECHAT_APP_ID="wx1234567890abcdef"
supabase secrets set WECHAT_APP_SECRET="your_app_secret_here"

# 3. 检查部署状态
supabase functions list
supabase secrets list
```

## 📞 技术支持

如果问题仍然存在，请提供以下信息：
1. 微信开发者工具控制台完整日志
2. Supabase Edge Function 日志
3. 网络请求详细信息
4. 错误发生的具体步骤